...
__global__ void q10_PA(DataGPU *dataGPU) {
    DuMatoGPU DM_GPU;
    DM_GPU.start_q10(dataGPU);
...
int main(int argc, const char** argv) {
    ...
    timerIO.play("IO");
    DuMatoCPU *DM_CPU = new DuMatoCPU(datasetName, k, numberOfActiveThreads, blockSize, numberOfSMs, jobsPerWarp, q10_PA, globalThreshold, relabeling, patternAware);
    Report* report;
    if(rep)
