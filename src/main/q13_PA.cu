...
__global__ void q13_PA(DataGPU *dataGPU) {
    DuMatoGPU DM_GPU;
    DM_GPU.start_q13(dataGPU);
...
int main(int argc, const char** argv) {
    ...
    timerIO.play("IO");
    DuMatoCPU *DM_CPU = new DuMatoCPU(datasetName, k, numberOfActiveThreads, blockSize, numberOfSMs, jobsPerWarp, q13_PA, globalThreshold, relabeling, patternAware);
    Report* report;
    if(rep)
