...
void Manager::organizeThreadStatus(std::vector<int>* idles, std::queue<Donator*>* actives, std::vector<int>* indifferents) {
    ...
    for(int i = 0, wid ; i < h_numberOfActiveThreads ; i+=h_virtualWarpSize) {
        // std::cout << i << "\n";
        wid = (int)(i/h_virtualWarpSize);
        if(h_status[i] == 2) {
            ...
            bool active = false;
            int offsetWarp = wid * h_virtualWarpSize;
...
void Manager::donate(std::vector<int>* idles, std::queue<Donator*>* actives) {
    ...
    for(int i = 0 ; i < idles->size() && !actives->empty() ; i++) {
        ...
        offsetWarpIdle = currentIdle*h_virtualWarpSize;
        offsetWarpDonator = donator->wid*h_virtualWarpSize;
...
void* Manager::reportFunction(Manager* manager) {
    ...
    while(!manager->gpuFinished) {
        ...
        {
            intraWarpIdle = 0;
            for(int j = 0 ; j < manager->h_virtualWarpSize ; j++) {
                if(status[i*manager->h_virtualWarpSize+j] == 2) {
                    amountThreadsIdle++;
                    intraWarpIdle++;
                }
            }
            if(intraWarpIdle == manager->h_virtualWarpSize)
                amountWarpsIdle++;
        }
        ...
        // printf("[REPORT] warps idle: %d, numberOfWarps: %d, %f warps active, %s", amountWarpsIdle, manager->numberOfWarps, 100-percentageWarpsIdle, std::asctime(std::localtime(&result)));
