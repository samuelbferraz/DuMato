...
DuMatoCPU::DuMatoCPU(const char *datasetName, int k, int numberOfActiveThreads, int blockSize, int numberOfSMs, int jobsPerWarp, void (*kernel)(DataGPU*), int globalThreshold, bool relabeling, int virtualWarpSize) {
    this->datasetName = datasetName;
    graphReader = new Graph(datasetName);
    dataCPU = new DataCPU;
    dataCPU->h_k = k;
    ...
    initializeCpuDataStructures(virtualWarpSize);
    initializeGpuDataStructures();
}
...
void DuMatoCPU::initializeCpuDataStructures(unsigned int virtualWarpSize) {
    /******************************************************************************************************/
    /*********************************************Constants************************************************/
    dataCPU->h_warpSize = virtualWarpSize;
    dataCPU->h_maxVertexId = graphReader->getMaxVertexId();
    dataCPU->h_numberOfEdges = graphReader->getNumberOfEdges();
    dataCPU->h_maxDegree = graphReader->getMaxDegree();
    dataCPU->h_maxDegreeRounded = dataCPU->h_maxDegree < dataCPU->h_warpSize ? dataCPU->h_warpSize : dataCPU->h_maxDegree;
...
void DuMatoCPU::initializeGpuDataStructures() {
    ...
    gpuErrorCheck(cudaMalloc((void**)&(dataGPU->d_warpSize), sizeof(unsigned int)));
    ...
    gpuErrorCheck(cudaMemcpy(dataGPU->d_warpSize, &(dataCPU->h_warpSize), sizeof(unsigned int), cudaMemcpyHostToDevice));
...
void DuMatoCPU::validateAggregateCounter() {
    ...
    if (!fp.is_open()) {
        std::cout << "Error opening the validation file of aggregate counter." << std::endl;
        return;
    }
...
void DuMatoCPU::validateAggregatePattern() {
    ...
    std::cout << "File for validation of aggregation: " << validationFile << "\n";
    ifstream fp(validationFile);

    if (!fp.is_open()) {
        std::cout << "Error opening the validation file of aggregate pattern." << std::endl;
        return;
    }
